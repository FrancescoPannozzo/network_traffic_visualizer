...
\section{Esecuzione del generatore e files prodotti}
...
\end{figure}
\section{Creazione della rete: links e distribuzione nodi}
Il generatore si serve di una serie di funzioni di utilità per creare le strutture in base alle scelte dell'utente. 
Tutte le tipologie di rappresentazioni avvengono seguendo una logica di distribuzione nodi in forma matriciale, fatta eccezione della tipologia di
grafo completo (spiegheremo nella parte dedicata al visualizzatore il perchè), come spiegheremo in dettaglio nel prossimo paragrafo.
...
Per ovviare al problema si è deciso di arrotondare il lato \(s\) all'intero superiore, così facendo sarà sempre verificato:
\begin{equation}
    (l+1)(h+1)>lh
\end{equation}
