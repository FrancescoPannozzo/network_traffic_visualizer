...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=0.6\textwidth,height=3.6cm,keepaspectratio]{immagini/torus_graph.png}
        \caption{Esempio di torus graph \cite{torusgraph}}
        \label{fig:torus_graph}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=0.7\textwidth,height=4cm,keepaspectratio]{immagini/GridGraph.jpg}
...
{\scriptsize % Cambia la dimensione del testo per il blocco lstlisting
\begin{lstlisting}[caption={Esempio di custom file per la configurazione}, label={codice:custom_graph_example}]
    ---
    data:
      graphType: mesh
      coordinates:
...
{\scriptsize % Cambia la dimensione del testo per il blocco lstlisting
\begin{lstlisting}[caption={Esempio di custom file con capacità uguali a tutti i link}, label={codice:custom_graph_example_only_links}]
data:
  graphType: torus
  coordinates:
    - [1, 0, 2, 0]
...
Un'altra possibilità è quella di lasciare che il programma ricavi in automatico i link (come spiegato in Sec. \ref{sec:creazione_links_nodi}), 
in questo caso basterà specificare solo linkCap che sarà uguale per tutti i link:

{\scriptsize % Cambia la dimensione del testo per il blocco lstlisting
\begin{lstlisting}[caption={Esempio di custom graph file con link automatici e stessa capacità}]
data:
  graphType: mesh
  coordinates:
    - [1, 2, 3]
...
un grafico che non sia di tipo mesh o torus, bisogna specificare nel campo \texttt{graphType} il valore "graph", mostriamo un esempio in Fig. \ref{codice:graphtype_graph}.

{\scriptsize % Cambia la dimensione del testo per il blocco lstlisting
\begin{lstlisting}[caption={Esempio di custom graph file, \texttt{graphType} posto al valore graph}, label={codice:graphtype_graph}]
    ---
    data:
      graphType: graph
      coordinates:
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Risultato grafico per un custom graph file}
        \label{fig:hybrid_custom_torus}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth]{immagini/free_graph.JPG}
        \caption{Risultato grafico per un custom graph file, modalità "graph"}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.48\textwidth]{immagini/diagramma_selezioni_config_gen.jpg}
...
\begin{figure}[h]
    ...
    \begin{minipage}{0.48\textwidth}
      ...
      \caption{Grafo mesh completo, opzione \texttt{dotsSize} posta a fixed}
      \label{fig:mesh_not_full}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
      \includegraphics[width=\linewidth]{immagini/adaptive_dots_mesh.JPG}
      \caption{Grafo mesh completo, opzione \texttt{dotsSize} posta ad adaptive}
...
troviamo che la complessità di questo pezzo logico di codice è \(O(p)\), fatta eccezione per l'opzione di grafo completo, poiché avremmo un numero di
...
in questo caso creiamo sempre una struttura ausiliaria necessaria a memorizzare man mano le somme pei payload. L'operazione 
consiste semplicemente in un ciclo for che scandisce la lista dei links presente nel network file, chiaramente le iterazioni dipendono dal numero di links
memorizzati, quindi ne deduciamo che il caricamento abbia complessità \(O(l)\), con \(l\) rappresentante il numero di links.
...
Segue un ciclo \texttt{for} che itera sui links, anche qui abbiamo una complessità di \(O(l)\). Dopo diverse operazioni con tempo O(c),
 la parte che merita attenzione è il ciclo \texttt{while} nidificato.
Questo ciclo itera su tutti i pacchetti che hanno un timestamp che ricade nell'unità frazionaria di tempo da analizzare, 
sono quindi tutti i pacchetti \(p\) in un dato intervallo di tempo \texttt{updateDelta} per un dato traffico percentuale;
la complessità dipende quindi dai pacchetti ottenendo \(O(p)\). Dopo diverse
operazioni con complessità O(c), i cicli che troviamo sono tre iterazioni sui links, ciascuno con complessità \(O(l)\).
...
Un'evenutale conferma arriva dai test dove possiamo vedere l'andamento lineare nel grafico nel caso medio ipotizzato 
precedentemente \ref{fig:prestazioni_analizzatore}. Possiamo
...
la funzione \lstinline|Graph()| per disegnare i grafi completi mentre abbiamo il codice visto in Fig. \ref{codice:grid_nodes_example} per i grafi mesh, torus e personalizzati.
Per quanto riguarda \lstinline|Graph()| rimane valida l'analisi di complessità quadratica per via del calcolo del numero di links in un grafo completo, mentre per la creazione delle altre tipologie di grafo
...
Il primo ciclo \texttt{while} itera su ciscuna unità frazionaria (ricordiamo definiamo con \(m\) la totalità delle unità frazionarie), vengono poi scanditi \(l\) links nel \texttt{for} interno andando a prendere il valore del traffico percentuale per l'unità frazionaria analizzata, abbiamo quindi \(O(l)\).
...
\begin{figure}[h]
    ...
    \caption{Comparazione prestazioni per risoluzioni di 480p, 720p e 1080p a 30 fps}
...
\begin{figure}[h]
    ...
    \begin{minipage}{0.48\textwidth}
      ...
      \caption{Test \texttt{json}, lettura e scrittura da 2 a 100 switch}
...
La natura del problema da affrontare ha reso necessario suddividere il progetto in tre parti, evidenziando una dimensione del dominio applicativo non di poco conto.
Nell'ottica di fornire un insieme coeso di caratteristiche possiamo riassumere quanto realizzato:

\begin{itemize}
    \item Un generatore di traffico fittizio di rete per scopi di testing e didattici, con capacità di creazione di reti in modo automatico
    oppure tramite parametri forniti dall'utente e che permette di utilizzare più formati.
    \item Un analizzatore di traffico di reti, con capacità di fornire le percentuali di traffico aggiornate per l'intero intervallo di tempo
    in cui è avvenuta la trasmissione.
    \item Un visualizzatore grafico con animazioni, con capacità di rappresentare diversi grafi mostrando visivamente l'intensità di traffico al variare del tempo.
\end{itemize}

Per quanto riguarda generatore, analizzatore e visualizzatore grafico, possiamo concludere che tutti i moduli del progetto
 operano in tempo lineare per il caso medio previsto, fatta eccezione della tipologia di grafo completo che intrinsicamente porta dietro di se una natura di complessità quadratica,
concludendo con un ottimo risultato in termini prestazionali.
Un'altra conclusione, non banale, riguarda l'essere riusciti a realizzare un prodotto di analisi di rete da una libreria
 che nasce con scopi relativi a video esplicativi matematici, come possono essere grafi cartesiani animati, spazi vettoriali animati e così via,
riuscendo quindi a sfruttare la libreria per fini non strettamente inerenti ai suoi scopi di esistenza. 
...
Possiamo proporre di seguito qualche esempio di dominio applicativo che possa essere sfruttato avendo l'applicazione del progetto come base:
...
\begin{itemize}
    ...
    dell'intensità del traffico stradale. In questo caso i nodi potrebbero essere punti d'interesse come le città, raggiungibili tramite strade (i links) e con i pacchetti che rappresentano i veicoli.
    ...
    \item \textbf{applicazioni distribuite:} in questo caso si potrebbe pensare a una rete composta dalle parti più piccole che compongono un'applicazione e collegate tra loro. La rappresentazione potrebbe interessare
