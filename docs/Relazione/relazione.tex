...
struttura seguente, con possibili varianti che elencheremo:
...
\section{Generatore di traffico e configurazione di rete}
...
aspetti nel capitolo dedicato allo studio della complessità computazionale e prestazioni.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.55\textwidth]{immagini/diagramma_selezioni_config_gen.jpg}
...
\subsection{Distribuzione spaziale dei nodi: aspect ratio}
Come appena descritto, la distribuzione dei nodi è pensata per essere disegnata in forma matriciale, questo segue una precisa scelta di design e riguarda
...
aventi capienza 1 Gbps, il numero dei link, per l'equazione in fig. \ref{eq:link_number}, risulta essere pari a 10. Supponiamo che tutta la rete
stia trasmettendo al 50\% della sua capacità trasmissiva per tutta la durata della stessa. Considerando una dimensione \(d\) di payload pari 
a 1518 Bytes,
definiamo come \(pps\) i pacchetti al secondo, da cui, per un link in un dato secondo avremo:
\begin{align*}
    \text{1 Gbps} &= 10^9 \text{ bit} && \text{(1 gigabit)} \\
    ...
    \text{bytes di differenza} &= 41172.59 \times 33 = 1358695.65 \text{ bytes} \approx 1.35 \text{ MBytes}\\
\end{align*}
Ciò significa che per ogni secondo di traffico abbiamo \(1.35 \text{ MB} \times 10\text{ links} = 13.5 \text{MB}\) in più, 
 in 100 secondi di traffico avremmo 1350 Mbytes, cioè 1.35 GBytes in più per una rete avente soltanto 10 links.
