...
il risultato del test sulle dimensioni dei files in figura \ref{fig:yaml_block_flow_dim} e quello relativo ai tempi di scrittura/lettura in 
figura \ref{fig:yaml_block_flow_wr}. Possiamo notare dei grafici che il risparmio di bytes è di circa 4 MB per il test effettuato su 10 switch, mentre
...
L'esecuzione dello script parte in primo luogo caricando i files richiesti e creando le strutture
...
La quale è esattamente l'equazione \(\frac{n(n-1)}{2}\) che determina il numero di links in un grafo completo. Con questo vogliamo dire che 
